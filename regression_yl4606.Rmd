...
top_wine_with_conti %>% 
  ...
  labs(x = 'Points', y = 'Log_Price', title = 'Scatter Plot - Log_Price vs. Points + Variety') +
...
### Final model fitting: Log_Price vs. Points + Variety + Continent
...
Yes. By doing an ANOVA test, we find that p-value equals to zero, which tells us that the larger model ' log_price ~ points + variety + continent ' is superior over 'log_price ~ points + variety', which means continent is a significant variable. Therefore, we get the following model :
...
For easier interpretation, we decide to stratify the analysis by continent.
...
After 1000 times bootstraps, we get a almost 20 percent lower in standard error for Pinot Noir estimate, which gives us more accurate confidence interval when predicting the price. 
